...
class Cart extends Component {
  ...
  handleSelected = (e, item) => {
    ...
    let temp = {
      id:item.id,
      subId:item.subId,
      subId2:e.target.value
    }
    this.props.displayModelInCart(temp);
  };
  handleNext = () => {
  ...
  handleRemove = (item) => {
    ...
    this.props.removeFromCart(temp);
  };
  handleAddQunatity = (item) => {
    let temp = {id:item.id,
      subId:item.subId,
    }
    this.props.addQuantity(temp);
  };
  handleSubtractQuantity = (item) => {
    let temp = {id:item.id,
      subId:item.subId
    }
    this.props.subtractQuantity(temp);
  };
          ...
          return (
              <li key={item.id} className="item-list">
                ...
                {/*})}*/}
                    ...
                    <div>
                      <select
                          // onClick={this.handleModelSelected(item)}
                        onChange={(e) => this.handleSelected(e,item)}
                        value={item.subId}
                    >
                      {item.models.map((x) => (
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    displayModelInCart: (id) => {
      dispatch(displayModelInCart(id));
    }
  };
};
